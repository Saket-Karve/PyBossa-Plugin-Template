    ...
    if current_user.is_authenticated():
    	...
    	if request.method == "POST":
    		...
    		#	session['quiz_id']=-1;
    			...
    			return redirect(url_for('quiz.display_result'))
    	else:
...
def display_result():
        total=len(models.response.query.filter_by(quiz_id=session['quiz_id']).all())
        correct=len(models.response.query.filter_by(result=True,quiz_id=session['quiz_id']).all())
